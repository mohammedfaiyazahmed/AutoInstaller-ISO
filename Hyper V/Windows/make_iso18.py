...
def run_powershell_script(script, as_admin=False):
    if as_admin:
        command = [
            ...
            '-Command',
            f'Start-Process powershell -ArgumentList \'-NoProfile -ExecutionPolicy Bypass -Command "{script}"\' -Verb RunAs'
        ]
    else:
        command = [
            'powershell.exe',
...
def add_autounattend_to_iso(iso_path, answer_file_path, new_iso_path):
    ...
    drive_letter_script = f'(Get-DiskImage -ImagePath "{iso_path}" | Get-Volume).DriveLetter'
