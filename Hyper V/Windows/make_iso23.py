...
def kill_imgburn():
    subprocess.run(['taskkill', '/F', '/IM', 'ImgBurn.exe'], check=True)
...
def unmount_iso(iso_path):
    unmount_command = ['PowerShell', '-Command', f'Dismount-DiskImage -ImagePath "{iso_path}"']
...
def main():
    try:
        ...
        pass  # Ignore errors if the ISO was not mounted

    try:
        kill_imgburn()
    except subprocess.CalledProcessError:
        pass  # Ignore errors if ImgBurn was not running
