...
def run_powershell_script(script, as_admin=False):
    if as_admin:
        ...
        completed = os.system(script)
    else:
        completed = os.popen(f'powershell -NoProfile -ExecutionPolicy Bypass -Command "{script}"').read()
    return completed
...
def add_autounattend_to_iso(iso_path, answer_file_path, new_iso_path):
    ...
    drive_letter = run_powershell_script('(Get-Volume -FileSystemLabel "CD-ROM").DriveLetter').strip() + ":\\"
